    ...
    public static class DatabaseHelper
    {
        ...
        public static void AddRoom(Room room)
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                ...
                string insertQuery = @"
                                    INSERT INTO Rooms (
                                        NameRoom, DescriptionRoom, PriceRoom, NumberRoom, FloorRoom, 
                                        TypeRoom, StatusRoom, Client, FirstName, LastName, 
                                        Email, Phone, PaymentMethod, IsPaid, TotalAmount, 
                ...
                using (var command = new SQLiteCommand(insertQuery, connection))
                {
                    command.Parameters.AddWithValue("@NameRoom", room.NameRoom ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@DescriptionRoom", room.DescriptionRoom ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@PriceRoom", room.PriceRoom);
                    command.Parameters.AddWithValue("@NumberRoom", room.NumberRoom);
                    command.Parameters.AddWithValue("@FloorRoom", room.FloorRoom ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@TypeRoom", (int)room.TypeRoom);
                    command.Parameters.AddWithValue("@StatusRoom", (int)room.StatusRoom);
                    command.Parameters.AddWithValue("@Client", room.Client ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@FirstName", room.FirstName ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@LastName", room.LastName ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@Email", room.Email ?? (object)DBNull.Value);
                    command.Parameters.AddWithValue("@Phone", room.Phone ?? (object)DBNull.Value);
                    ...
                    command.Parameters.AddWithValue("@CheckInDate",
                        room.CheckInDate.HasValue ? (object)room.CheckInDate.Value.ToString("yyyy-MM-dd HH:mm:ss") : DBNull.Value);
                    command.Parameters.AddWithValue("@CheckOutDate",
                        room.CheckOutDate.HasValue ? (object)room.CheckOutDate.Value.ToString("yyyy-MM-dd HH:mm:ss") : DBNull.Value);
